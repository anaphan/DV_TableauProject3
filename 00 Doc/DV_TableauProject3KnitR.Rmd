...
Causes of death are already grouped by ages and separated individually by race, so that each cause has a value for white, other, black and hispanic in certain age groups. We chose "Black" to show all the causes of death for a single race and made a calculated field of black birth defects, black cancer, etc. with all ages. 
...
For our final visualization, it will be similar to the previous in using the causes of death but instead separating by filtering for an age group for individuals under 19. Based on the bar graph we see that individuals under 19 are dying mostly from cancer and suicide compared to other causes. 
